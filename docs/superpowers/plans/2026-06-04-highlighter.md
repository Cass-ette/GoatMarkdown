...
final class HighlightStoreTests: XCTestCase {
    ...
    func testSetEmptyHighlightsRemovesFileEntry() {
        let store = HighlightStore(defaults: defaults)
        store.setHighlights([Highlight(blockIndex: 0, rangeStart: 0, rangeEnd: 5, color: .green)], for: "/tmp/A.md")
...
final class HighlightState {
    ...
    func removeAll(in blockIndex: Int, range: Range<Int>) {
        highlights.removeAll { existing in
    ...
    func find(blockIndex: Int, range: Range<Int>) -> Highlight? {
    ...
    func toggle(blockIndex: Int, range: Range<Int>, color: HighlightColor = .yellow) -> Highlight? {
        if let existing = find(blockIndex: blockIndex, range: range) {
    ...
    func highlights(in blockIndex: Int) -> [Highlight] {
        highlights.filter { $0.blockIndex == blockIndex }
    }
...
final class HighlightStateTests: XCTestCase {
    ...
    func testAddAppendsHighlight() {
        let h = Highlight(blockIndex: 0, rangeStart: 0, rangeEnd: 5, color: .yellow)
        state.add(h)
    ...
    func testAddWithSameIDReplaces() {
        let id = UUID()
        let first = Highlight(id: id, blockIndex: 0, rangeStart: 0, rangeEnd: 5, color: .yellow)
        let second = Highlight(id: id, blockIndex: 1, rangeStart: 0, rangeEnd: 5, color: .blue)
        state.add(first)
        state.add(second)
    ...
    func testFindReturnsOverlappingHighlight() {
        let h = Highlight(blockIndex: 0, rangeStart: 5, rangeEnd: 15, color: .yellow)
        state.add(h)
        let found = state.find(blockIndex: 0, range: 10..<12)
        XCTAssertEqual(found?.id, h.id)
    }

    func testFindReturnsNilForNonOverlapping() {
        state.add(Highlight(blockIndex: 0, rangeStart: 5, rangeEnd: 15, color: .yellow))
        XCTAssertNil(state.find(blockIndex: 0, range: 20..<25))
    }

    func testToggleAddsWhenNoExisting() {
        let result = state.toggle(blockIndex: 0, range: 0..<5)
        XCTAssertNotNil(result)
        XCTAssertEqual(state.highlights.count, 1)
    }

    func testToggleRemovesWhenExisting() {
        _ = state.toggle(blockIndex: 0, range: 0..<5)
        let result = state.toggle(blockIndex: 0, range: 0..<5)
        XCTAssertNil(result)
        XCTAssertTrue(state.highlights.isEmpty)
    }

    func testRemoveAllInRangeRemovesOverlapping() {
        state.add(Highlight(blockIndex: 0, rangeStart: 0, rangeEnd: 10, color: .yellow))
        state.add(Highlight(blockIndex: 0, rangeStart: 20, rangeEnd: 30, color: .blue))
        state.removeAll(in: 0, range: 5..<25)
        XCTAssertEqual(state.highlights.count, 1)
        XCTAssertEqual(state.highlights.first?.color, .blue)
    }

    func testHighlightsInBlockFiltersByIndex() {
        state.add(Highlight(blockIndex: 0, rangeStart: 0, rangeEnd: 5, color: .yellow))
        state.add(Highlight(blockIndex: 1, rangeStart: 0, rangeEnd: 5, color: .green))
        XCTAssertEqual(state.highlights(in: 0).count, 1)
        XCTAssertEqual(state.highlights(in: 1).count, 1)
        XCTAssertEqual(state.highlights(in: 2).count, 0)
    }

    func testLoadReplacesAll() {
        state.add(Highlight(blockIndex: 0, rangeStart: 0, rangeEnd: 5, color: .yellow))
        let replacement = [Highlight(blockIndex: 1, rangeStart: 0, rangeEnd: 5, color: .blue)]
...
```swift
    func toggleHighlight(blockIndex: Int, range: Range<Int>) {
        _ = highlightState.toggle(blockIndex: blockIndex, range: range)
        saveHighlights()
    }
```
...
```swift
    private func applyHighlights(
        ...
        let highlights = highlightState.highlights(in: blockIndex)
...
With:
```swift
        let highlightFingerprint = highlightState?.highlights(in: blockIndex ?? -1).map { "\($0.id.uuidString)|\($0.rangeStart)|\($0.rangeEnd)|\($0.color.rawValue)" }.joined(separator: ",") ?? ""
        if let query = searchState?.query, !query.isEmpty {
            let currentMatchSignature = searchState?.currentMatch?.signature ?? "none"
...
- [ ] **Step 3: Add Cmd+Shift+H handler**
...
- [ ] **Step 4: Add applyHighlightFromSelection helper**

After `openInNewWindowPanel()` method (line 234-244), add:
```swift
    private func applyHighlightFromSelection(in state: MarkdownReaderState) {
        // MVP: get current NSTextView selection and apply yellow highlight
        guard let textView = NSApp.keyWindow?.firstResponder as? NSTextView else { return }
        let selectedRanges = textView.selectedRanges
        guard let firstRange = selectedRanges.first?.rangeValue else { return }
        guard firstRange.location != NSNotFound, firstRange.length > 0 else { return }

        // Map selected text to a block by finding which block contains the selected text
        // For MVP: assume single-block selection and use first matched block
        let selectedString = textView.string as NSString
        let selectedText = selectedString.substring(with: firstRange)
        ...
        for (blockIndex, block) in document.blocks.enumerated() {
            let blockTexts = extractBlockTexts(from: block)
            for (_, blockText) in blockTexts.enumerated() {
                let blockTextLength = (blockText as NSString).length
                let blockStart = characterOffset
                let blockEnd = characterOffset + blockTextLength

                // Check if selection overlaps this block
                if firstRange.location < blockEnd && firstRange.location + firstRange.length > blockStart {
                    let localStart = max(0, firstRange.location - blockStart)
                    let localEnd = min(blockTextLength, firstRange.location + firstRange.length - blockStart)
                    if localStart < localEnd {
                        state.toggleHighlight(blockIndex: blockIndex, range: localStart..<localEnd)
                    }
                    return
                }
                characterOffset = blockEnd
            }
        }
    }

    private func extractBlockTexts(from block: MarkdownBlock) -> [String] {
        switch block {
